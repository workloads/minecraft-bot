...
module.exports = (bot: mineflayer.Bot) => {
    bot.addChatPattern("status", new RegExp(`<(.*)> ${instance.getSettings().commandsPrefix}status`), { repeat: true, parse: true });
    ...
    bot.on("chat:status" as keyof BotEvents, async ([[username]]: string) => {
        ...
        const hungerc = bot.food > 15 ? "§a" : (bot.food <= 15 && bot.food > 9) ? "§6" : bot.food < 9 ? "§c" : null;
        const healthc = bot.health > 15 ? "§a" : (bot.health <= 15 && bot.health > 9) ? "§6" : bot.health < 9 ? "§c" : null;
