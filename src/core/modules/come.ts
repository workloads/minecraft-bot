...
module.exports = (bot: mineflayer.Bot) => {
    bot.addChatPattern("come", new RegExp(`<(.*)> ${instance.getSettings().commandsPrefix}(come|come to me)`), { repeat: true, parse:true });
    ...
    bot.on("chat:come" as keyof BotEvents, ([[username]] : [string]): void => {
        ...
        instance.sendToDiscord(`come command was executed by ${username}.`);
